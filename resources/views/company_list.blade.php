...
<div class="listView">
  ...
  <div class="filter-bar">
        <div class="left">
            <input type="text" name="search" placeholder="Search by name, industry, size..." value="{{ request('search') }}">
            <button type="submit">Filter By</button>
        </div>
        <div class="right">
            <select name="filter_by">
                <option value="">Select filter</option>
                <option value="Company Name" {{ request('filter_by') == 'Company Name' ? 'selected' : '' }}>Company Name</option>
                <option value="Industry" {{ request('filter_by') == 'Industry' ? 'selected' : '' }}>Industry</option>
                <option value="Size" {{ request('filter_by') == 'Size' ? 'selected' : '' }}>Size</option>
                <option value="Rating" {{ request('filter_by') == 'Rating' ? 'selected' : '' }}>Rating</option>
            </select>
            <button type="submit">
                <!-- Your SVG icon for the date picker or other action -->
                13 Jan, 2024
            </button>
            <button type="button" id="toggleCSV">
                <!-- Your SVG icon for export CSV -->
                Export CSV
            </button>
        </div>
    </div>
    ...
    <div class="table-container">
        <table>
            ...
            <tr>
                ...
                <td>{{ $company->size }}</td>
                <td>
                    <span class="status {{ $company->collaboration_history }}">{{ ucfirst($company->collaboration_history) }}</span>
                </td>
...
document.addEventListener("DOMContentLoaded", function() {
        ...
        var toggleCSVButton = document.getElementById("toggleCSV");

        // Add an event listener for the button click
        toggleCSVButton.addEventListener("click", function() {
            exportCSV();
        });
        ...
        function exportCSV() {
            ...
            form.method = 'GET'; // Use GET or POST based on your backend route
            form.action = '{{ route('companies.export') }}'; // Update with your route for exporting CSV

            // Add the search and filter values as hidden inputs
            ...
            var filterByInput = document.createElement('input');
            filterByInput.type = 'hidden';
            filterByInput.name = 'filter_by';
            filterByInput.value = document.querySelector('select[name="filter_by"]').value;
            form.appendChild(filterByInput);
